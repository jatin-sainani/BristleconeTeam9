...
class BackgroundCollectedPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    DateTime beginningArgumentsStep = DateTime(
      beginningArguments.year, beginningArguments.month, beginningArguments.day
    );
    ...
    final Iterable<LabelEntry> argumentsLabels = argumentsLabelsTimestamps.map((timestamp) {
      return LabelEntry(
        ...
        (
          (timestamp.hour <= 9 ? '0' : '') + timestamp.hour.toString() + 
            ':' + 
          (timestamp.minute <= 9 ? '0' : '') + timestamp.minute.toString()
        )
      );
    });
